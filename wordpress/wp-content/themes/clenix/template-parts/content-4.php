...
$clenix_has_entry_meta  = ( ClenixTheme::$options['blog_date'] || ClenixTheme::$options['blog_cats'] || ClenixTheme::$options['blog_comment_num'] || ClenixTheme::$options['blog_view'] && function_exists( 'clenix_views' ) || ClenixTheme::$options['blog_length'] && function_exists( 'clenix_reading_time' ) ) ? true : false;
...
if ( ( !has_post_thumbnail() ) && ClenixTheme::$options['display_no_preview_image'] == '0' ) {
	...
	$post_layout_class = 'col-lg-6 col-md-12';	
}
if ( ClenixTheme::$layout != 'full-width' ) {	
	$post_image = 'col-lg-6 col-md-12 col-sm-12 col-12';	
} else {
	$post_image = 'col-lg-6 col-md-6 col-sm-4 col-12';
}
...
<div id="post-<?php the_ID(); ?>" <?php post_class( $post_classes ); ?>>
	<div class="blog-box">
		<div class="row no-gutters">
			<?php if ( $i % 2 == 1 ) { ?>
			<div class="<?php echo esc_attr( $post_layout_class ); ?> entry-content order2">
				<div class="inner-content">
					<h3><a href="<?php the_permalink();?>"><?php the_title();?></a></h3>
					<?php if ( ClenixTheme::$options['blog_author_name'] ) { ?>
						<span class="item-author"><i class="fa fa-user" aria-hidden="true"></i><?php esc_html_e( 'by ', 'clenix' );?><?php the_author_posts_link(); ?></span>
					<?php } ?>
			...
			<div class="<?php echo esc_attr( $post_image ); ?> entry-image">
				<div class="blog-img-holder">
					<div class="blog-img">
						<a href="<?php the_permalink(); ?>" class="img-opacity-hover"><?php if ( has_post_thumbnail() ) { ?>
							<?php the_post_thumbnail( 'clenix-size6', ['class' => 'img-responsive'] ); ?>
							<?php } else {
							if ( ClenixTheme::$options['display_no_preview_image'] == '1' ) {
								...
								elseif ( empty( ClenixTheme::$options['no_preview_image']['id'] ) ) {
									$thumbnail = '<img class="wp-post-image" src="'.CLENIX_IMG_URL.'noimage_610X580.jpg" alt="'. the_title_attribute( array( 'echo'=> false ) ) .'">';
								}
