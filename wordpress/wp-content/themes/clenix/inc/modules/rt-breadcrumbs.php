 ...
 */
 ...
 /*CREDIT : Modified By Parvez*/
 
if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly

if( ! function_exists( 'clenix_breadcrumbs' )){

	function clenix_breadcrumbs(){
			
		if( ClenixTheme::$options['breadcrumb_active'] == '0' || ClenixTheme::$options['breadcrumb_hide_mobile'] == '0' ) {			
			return;
		}

		# breadcrumbs ----------
		$delimiter  = ClenixTheme::$options['breadcrumbs_delimiter'] ? wp_kses_post( ClenixTheme::$options['breadcrumbs_delimiter'] ) : '&#47;';
		$delimiter  = '<em class="delimiter">'. $delimiter .'</em>';

		$home_text  = esc_html__( 'Home', 'clenix' );
		...
		if ( ! is_home() && ! is_front_page() || is_paged() ){
			...
			elseif ( is_singular() ){			
				...
				if ( get_post_type() == 'post' ){
					...
					$category = clenix_get_primary_category_id();
					$category = get_category( $category );
